...
# 测试4: 检查是否可以修改角色
print("[*] 测试4: 检查是否可以修改角色")

# 尝试修改用户的角色为 admin
update_resp = session.post(f"{BASE_URL}/api/update", 
    json={"uid": uid, "role": "admin"},
    headers={"Content-Type": "application/json"})
print(f"    POST /api/update: {update_resp.status_code}")
...
profile_resp = session.post(f"{BASE_URL}/api/profile", 
    json={"uid": uid},
    headers={"Content-Type": "application/json"})
print(f"    新的 profile: {profile_resp.text}")
